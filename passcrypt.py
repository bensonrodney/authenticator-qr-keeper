...
if __name__ == "__main__":
    ...
    with TemporaryDirectory(prefix="passcrypt_") as tmpdir:
        ...
        save_data_to_encrypted_file(
            str(python_encrypted_file), plaintext.encode(), password
        )
        ...
        python_decrypted_file_bytes = load_data_from_encrypted_file(
            str(python_encrypted_file), password
        )

        # create encrypted file using openssl and decrypt in python
        openssl_encrypted_file = Path(tmpdir) / "openssl_encrypted_file"
        result2 = subprocess.run(
            f'echo "{password}" | openssl aes-256-cbc -pass stdin -pbkdf2 -iter 10000 -salt -in {str(plaintext_file)} '
            f'-out {str(openssl_encrypted_file)}',
            shell=True,
            check=False,
            stdout=subprocess.PIPE,
        ...
        python_decrypted_file_bytes2 = load_data_from_encrypted_file(
            str(openssl_encrypted_file), password
        )
